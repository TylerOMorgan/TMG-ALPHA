...
const GoogleDriveIcon: React.FC<{ className?: string }> = ({ className }) => (
  ...
  >
    ...
    <path
      d="M154.1 67.4L0 338.5 80.6 480 237 208.8 154.1 67.4z"
      fill="#00AC47"
    />
...
const getDemoLinkIcon = (url: string) => {
  ...
  if (clean.includes("soundcloud")) {
    return (
      <SoundCloudIcon className="w-5 h-5 text-[#FF5500] shrink-0 drop-shadow-[0_0_8px_rgba(255,85,0,0.4)] transition-all duration-300 animate-in fade-in zoom-in-75" />
    );
  }
  if (clean.includes("tiktok")) {
    return (
      <TikTokIcon className="w-5 h-5 text-white shrink-0 filter drop-shadow-[-1.5px_0px_0_#25F4EE] drop-shadow-[1.5px_0px_0_#FE2C55] transition-all duration-300 animate-in fade-in zoom-in-75" />
    );
  }
  if (
    clean.includes("drive.google") ||
    clean.includes("docs.google") ||
    clean.includes("google.com/drive")
  ) {
    return (
      <GoogleDriveIcon className="w-5 h-5 shrink-0 drop-shadow-[0_0_8px_rgba(66,133,244,0.3)] transition-all duration-300 animate-in fade-in zoom-in-75" />
    );
  }
  return (
    <Link className="w-5 h-5 text-white/30 group-focus-within/input:text-trillex-orange transition-colors shrink-0" />
  );
};
  ...
  return (
    ...
    >
      ...
      {active && (
        <Check
          size={18}
          className="relative z-10 text-trillex-orange shrink-0 ml-3"
        />
      )}
    </li>
  );
...
const CustomSelect: React.FC<CustomSelectProps> = ({
  ...
  const isCustomActive = Boolean(
    allowCustom && value && value !== "Select" && !isPredefined,
  );
  ...
  const getSelectedDisplay = () => {
    ...
    return (
      <span className="flex items-center gap-2">
        <span>{value}</span>
        <span className="text-[10px] font-mono uppercase px-1.5 py-0.5 rounded bg-trillex-orange/20 text-trillex-orange border border-trillex-orange/30">
          Custom
        </span>
      </span>
    );
  ...
  return (
    <div
      className={`flex flex-col gap-2 relative transition-all duration-150 ${isOpen ? "z-50" : "z-20"}`}
      ref={containerRef}
    >
      <label className="text-xs font-mono text-white/70 uppercase tracking-wider pl-1">
        {label}
        <span className="text-trillex-orange">*</span>
      </label>
      ...
      <div className="relative">
        ...
        >
          <span
            className={`${value === "Select" ? "text-white/50" : "text-white"}`}
          >
            ...
            >
              {filteredOptions.length > 0 ? (
                filteredOptions.map((option) => {
                  ...
                  const isActive = isOthersOption
                    ? isCustomActive || value === "Others"
                    : value === optValue;
                      ...
                      <div className="flex items-center justify-between w-full gap-4 sm:gap-6">
                        <div className="flex items-center gap-2.5">
                          ...
                          {isOthersOption && isCustomActive && (
                            <span className="text-xs font-mono text-trillex-orange">
                              ({value})
                            </span>
                          )}
                        </div>
...
const ContactForm: React.FC<{ isActive?: boolean }> = ({ isActive = true }) => {
  ...
  useEffect(() => {
    const handleHashSync = () => {
      const hash = window.location.hash.toLowerCase().replace(/^#\/?/, "");
      if (
        ["demo-submission", "demo-submissions", "demo", "demos"].includes(hash)
      ) {
        if (activeTab !== "demo") {
  ...
  // Update page title when tab changes (only while Contact page is visible —
  // all pages stay mounted, so an unconditional set would stomp other pages)
  useEffect(() => {
    if (isActive) {
      document.title =
        activeTab === "demo"
          ? "DEMO SUBMISSION — TRILLEX"
          : "GENERAL INQUIRY — TRILLEX";
    }
  }, [activeTab, isActive]);
  ...
  useEffect(() => {
    ...
    const handleSetSubLabel = (e: Event) => {
      ...
      if (customEvent.detail?.subLabel) {
        setDemoForm((prev) => ({
          ...prev,
          subLabel: customEvent.detail.subLabel,
          genre: customEvent.detail.subLabel,
        }));
      }
    };
    window.addEventListener("trillex-set-sublabel", handleSetSubLabel);
    return () =>
      window.removeEventListener("trillex-set-sublabel", handleSetSubLabel);
  }, []);
  ...
  const handleTabChange = (tab: "general" | "demo", updateUrl = true) => {
    ...
    if (updateUrl) {
      const targetHash =
        tab === "demo" ? "#demo-submission" : "#general-inquiry";
  ...
  const handleDemoSubmit = async (e: React.FormEvent) => {
    ...
    const isValidLink =
      /^https?:\/\/(?:[a-zA-Z0-9-]+\.)*(soundcloud\.com|tiktok\.com|drive\.google\.com|docs\.google\.com)(\/.*)?$/i.test(
        link,
      );
    ...
    if (!isValidLink) {
      showToast(
        "Invalid Link",
        "Must be a SoundCloud, TikTok, or Google Drive link",
        "error",
      );
      return;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-start">
        <div className="flex flex-col pt-4 items-center text-center lg:items-start lg:text-left">
          ...
          <p className="hidden lg:block text-lg md:text-xl text-white/60 font-light mb-6 max-w-lg lg:max-w-none">
            Thank you for your interest in Trillex Music Group! After
            submission, we will contact you within 48 hours.
          </p>
          ...
          <div className="hidden lg:flex flex-col gap-4 mt-8 lg:mt-10 w-full max-w-sm">
            ...
            <div className="p-6 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-md transition-all duration-300 hover:bg-white/10 group">
              ...
              <div className="flex flex-col gap-3">
                {[
                  {
                    handle: "@trillexmusicgroup",
                    url: "https://www.instagram.com/trillexmusicgroup",
                  },
                  {
                    handle: "@trillexbounce",
                    url: "https://www.instagram.com/trillexbounce",
                  },
                  {
                    handle: "@trillexavant",
                    url: "https://www.instagram.com/trillexavant",
                  },
                ].map((item, index) => (
                  <a
                    key={index}
        ...
        <div className="w-full bg-[#0A0A0A] border border-white/10 rounded-3xl p-6 md:p-10 shadow-2xl relative overflow-hidden group">
          ...
          >
            ...
            <CustomSelect
              label="Genre"
              value={
                demoForm.genre !== "Select" ? demoForm.genre : demoForm.subLabel
              }
              onChange={(value) =>
                setDemoForm({ ...demoForm, genre: value, subLabel: value })
              }
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-mono text-white/70 uppercase tracking-wider pl-1">
                Artist Name<span className="text-trillex-orange">*</span>
              </label>
              <div className="relative group/input">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30 group-focus-within/input:text-trillex-orange transition-colors w-5 h-5 pointer-events-none" />
                <input
                  ...
                  onChange={(e) =>
                    setDemoForm({ ...demoForm, artistName: e.target.value })
                  }
                  className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-4 text-base text-white placeholder-white/20 focus:border-trillex-orange/50 focus:bg-white/10 focus:ring-1 focus:ring-trillex-orange/50 outline-none transition-all duration-300"
                />
              </div>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-mono text-white/70 uppercase tracking-wider pl-1">
                Song Title<span className="text-trillex-orange">*</span>
              </label>
              <div className="relative group/input">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDemoForm({ ...demoForm, songTitle: e.target.value })
                  }
                  className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-4 text-base text-white placeholder-white/20 focus:border-trillex-orange/50 focus:bg-white/10 focus:ring-1 focus:ring-trillex-orange/50 outline-none transition-all duration-300"
                />
              </div>
            ...
            <CustomSelect
              ...
              onChange={(value) =>
                setDemoForm({ ...demoForm, isCollab: value })
              }
              options={["Yes", "No"]}
              icon={Users}
              onMouseEnter={disableCursor}
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-mono text-white/70 uppercase tracking-wider pl-1">
                Demo Link (SoundCloud / TikTok / Google Drive)
                <span className="text-trillex-orange">*</span>
              </label>
              <div className="relative group/input">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDemoForm({ ...demoForm, demoLink: e.target.value })
                  }
                  onInvalid={(e) =>
                    (e.target as HTMLInputElement).setCustomValidity(
                      "Please provide a valid SoundCloud, TikTok, or Google Drive link.",
                    )
                  }
                  onInput={(e) =>
                    (e.target as HTMLInputElement).setCustomValidity("")
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-mono text-white/70 uppercase tracking-wider pl-1">
                Contact Email<span className="text-trillex-orange">*</span>
              </label>
              <div className="relative group/input">
                <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30 group-focus-within/input:text-trillex-orange transition-colors w-5 h-5 pointer-events-none" />
                <input
                  ...
                  onChange={(e) =>
                    setDemoForm({ ...demoForm, contactEmail: e.target.value })
                  }
                  className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-4 text-base text-white placeholder-white/20 focus:border-trillex-orange/50 focus:bg-white/10 focus:ring-1 focus:ring-trillex-orange/50 outline-none transition-all duration-300"
                />
              </div>
            ...
            <CustomSelect
              ...
              onChange={(value) =>
                setDemoForm({ ...demoForm, hasProfile: value })
              }
              options={["Yes", "No"]}
              icon={User}
              onMouseEnter={disableCursor}
              ...
              <div className="flex flex-col gap-2 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="relative group/input">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDemoForm({ ...demoForm, profileLink: e.target.value })
                    }
                    className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-4 text-base text-white placeholder-white/20 focus:border-trillex-orange/50 focus:bg-white/10 focus:ring-1 focus:ring-trillex-orange/50 outline-none transition-all duration-300"
                  />
                </div>
  ...
  return (
    <section
      id="contact-form"
      ref={containerRef}
      className="relative bg-trillex-black py-5 md:py-10 overflow-hidden flex flex-col items-center"
    >
      {/* Tab Buttons */}
      ...
      <div
        ref={formWrapperRef}
        className="container mx-auto px-4 md:px-12 relative z-10 opacity-0 pb-20"
      >
        <div ref={contentRef}>
      ...
      <div
        className={`fixed top-32 left-1/2 -translate-x-1/2 z-[10000] pointer-events-none transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${toast.show ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-8"}`}
      >
        <div
          className={`bg-black border ${toast.type === "error" ? "border-red-500/50" : "border-white/20"} text-white px-8 py-5 min-w-[350px] rounded-full shadow-[0_10px_40px_rgba(0,0,0,0.8)] flex items-center gap-5`}
        >
          <div
            className={`w-10 h-10 shrink-0 rounded-full ${toast.type === "error" ? "bg-red-500" : "bg-trillex-orange"} flex items-center justify-center text-black shadow-[0_0_15px_rgba(255,127,80,0.6)]`}
          >
            {toast.type === "error" ? (
          ...
          <div className="flex flex-col">
            <span className="font-display font-bold text-lg tracking-wide uppercase leading-tight">
              {toast.title}
            </span>
            <span className="font-mono text-[11px] text-white/50 uppercase tracking-widest leading-tight mt-1">
              {toast.subtitle}
            </span>
          </div>
        </div>
