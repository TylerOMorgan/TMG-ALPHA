...
import rawSvg from "../public/ecosystem.svg?raw";
...
const fetchSvg = async (): Promise<string> => {
  ...
  if (!fetchPromise) {
    const urls = [
      "/ecosystem.svg?v=v5",
      "/Trillex%20Website%20SVG%20Animations%20-%20Edit.svg?v=v5",
      "/Trillex Website SVG Animations - Edit.svg?v=v5",
      "/Trillex%20Website%20SVG%20Animations.svg?v=v5",
    ];
    fetchPromise = (async () => {
...
const SvgAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Cache live animation handles once on mount rather than querying DOM on every scroll frame
    ...
    const timer1 = setTimeout(syncLayout, 100);
