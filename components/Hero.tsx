...
// --- CONFIGURATION ---
const DESKTOP_TEXTURE_URL = "/HeroSectionV5.webp";
const TABLET_TEXTURE_URL = "/HeroSectionTablet.webp";
const MOBILE_TEXTURE_URL = "/HeroSectionMobile.webp";

// Preload textures to minimize swap lag (THREE.TextureLoader via R3F suspense cache)
[DESKTOP_TEXTURE_URL, TABLET_TEXTURE_URL, MOBILE_TEXTURE_URL].forEach((url) => {
...
const SceneContent = () => {
  ...
  useFrame((state) => {
    ...
    const lubVal =
      smoothstep(0.0, 0.1, beatTime) * smoothstep(0.4, 0.15, beatTime);
    const dubVal =
      smoothstep(0.4, 0.6, beatTime) * smoothstep(1.0, 0.5, beatTime);
...
const Hero: React.FC = () => {
  ...
  return (
    <div
      id="home"
      className="relative w-full h-[100vh] supports-[height:100dvh]:h-[100dvh] min-h-[500px] md:min-h-[600px] overflow-hidden bg-trillex-black"
    >
      ...
      <div
        ref={textRef}
        className="absolute inset-0 z-10 flex flex-col items-center justify-center pointer-events-none mix-blend-exclusion will-change-transform px-4 text-center"
      >
        ...
        >
          ...
          <div className="relative flex items-center justify-center gap-2 z-10">
            ...
            <div className="relative overflow-hidden w-4 h-4 md:w-5 md:h-5">
              <ArrowUpRight
                strokeWidth={3}
                className="absolute inset-0 w-full h-full text-white transition-all duration-500 ease-[cubic-bezier(0.19,1,0.22,1)] md:group-hover:translate-x-full md:group-hover:-translate-y-full"
              />
              <ArrowUpRight
                strokeWidth={3}
                className="absolute inset-0 w-full h-full text-black transition-all duration-500 ease-[cubic-bezier(0.19,1,0.22,1)] -translate-x-full translate-y-full hidden md:block md:group-hover:translate-x-0 md:group-hover:translate-y-0"
              />
            </div>
          </div>
        </button>
